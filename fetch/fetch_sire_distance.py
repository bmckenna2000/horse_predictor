...
OUTPUT_FILE = os.path.join(DATA_DIR, "sire_distance_stats.csv")

# ✅ Define API endpoint
API_URL_TEMPLATE = f"{BASE_URL}/sires/{{sire_id}}/analysis/distances"
...
def fetch_sire_stats(sire_id):
    ...
    try:
        ...
        if not distances:
            print(f"⚠️ No distance data found for sire: {sire_id}")
            return []
        ...
        print(f"✅ Successfully fetched stats for sire: {sire_id} ({len(stats)} distances)")
...
if not df.empty:
    ...
    print(f"✅ Saved {len(df)} sire distance stats to {OUTPUT_FILE}")
else:
