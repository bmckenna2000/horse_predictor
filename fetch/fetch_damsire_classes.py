...
OUTPUT_FILE = os.path.join(DATA_DIR, "damsire_class_stats.csv")

# ✅ Define API endpoint
API_URL_TEMPLATE = f"{BASE_URL}/damsires/{{damsire_id}}/analysis/classes"

# ✅ Debugging: Print BASE_URL and sample URL
print(f"🔍 BASE_URL: {BASE_URL}")
print(f"🔍 Sample API URL Template: {API_URL_TEMPLATE}")

# ✅ Load damsire IDs
damsire_df = pd.read_csv(DAMSIRE_IDS_FILE)
damsire_ids = damsire_df["damsire_id"].unique()

# ✅ Debugging: Print sample damsire IDs
print(f"🔍 Total unique damsire_ids: {len(damsire_ids)}")
print(f"🔍 Sample damsire_ids: {list(damsire_ids[:5])}")  # Show first 5 IDs
...
def fetch_damsire_stats(damsire_id):
    ...
    print(f"📡 Fetching data for damsire: {damsire_id}... URL: {url}")  # Added URL to debug
    ...
    try:
        ...
        if not classes:
            print(f"⚠️ No class data found for damsire: {damsire_id}")
            return []
        ...
        print(f"✅ Successfully fetched stats for damsire: {damsire_id} ({len(stats)} classes)")
...
if not df.empty:
    ...
    print(f"✅ Saved {len(df)} damsire class stats to {OUTPUT_FILE}")
else:
