...
OUTPUT_FILE = os.path.join(DATA_DIR, "jockey_trainer_stats.csv")
...
# Fetch Jockey-Trainer Stats with retry logic
@retry(
...
def fetch_jockey_trainer_stats(jockey_id):
    """Fetch trainer performance stats for a jockey."""
    url = f"{BASE_URL}/jockeys/{jockey_id}/analysis/trainers"
    print(f"📡 Fetching trainer stats for jockey {jockey_id}...")
    ...
    if "trainers" in data and data["trainers"]:
        stats = [{"jockey_id": jockey_id, "total_rides": total_rides, **trainer} 
                 for trainer in data["trainers"]]
        print(f"✅ Jockey {jockey_id} - {len(stats)} trainers found! (Total Rides: {total_rides})")
        return stats

    print(f"⚠️ No trainer stats found for jockey {jockey_id}")
    return None
...
def main():
    ...
    if os.path.exists(OUTPUT_FILE):
        ...
        df_existing = pd.DataFrame(columns=["jockey_id", "total_rides", "trainer_id", "trainer", 
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_jockey = {executor.submit(fetch_jockey_trainer_stats, jockey_id): jockey_id 
    ...
    if df_list:  # If there's valid data, concatenate
        df_updated = pd.concat(df_list).drop_duplicates(subset=["jockey_id", "trainer_id"], keep="last")
    else:
    ...
    if not df_updated.empty:
        ...
        print(f"✅ Saved {len(df_updated)} jockey trainer stats to {OUTPUT_FILE}")
    else:
        print("⚠️ No jockey trainer stats found.")
