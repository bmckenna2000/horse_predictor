...
OUTPUT_FILE = os.path.join(DATA_DIR, "trainer_course_stats.csv")  # Output: Trainer course stats
...
# ✅ Fetch Trainer Course Stats
def fetch_trainer_course_stats(trainer_id):
    """Fetch course performance stats for a trainer."""
    url = f"{BASE_URL}/trainers/{trainer_id}/analysis/courses"
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            time.sleep(2)
            return fetch_trainer_course_stats(trainer_id)  # Retry
        ...
        if "courses" in data and data["courses"]:
            return [{"trainer_id": trainer_id, "total_rides": total_rides, **course} for course in data["courses"]]

    except Exception as e:
        print(f"❌ Error fetching trainer course stats: {e}")
...
def main():
    ...
    for trainer_id in trainer_ids:
        print(f"📡 Fetching course stats for trainer {trainer_id}...")

        trainer_data = fetch_trainer_course_stats(trainer_id)

        if trainer_data:
            results.extend(trainer_data)
            print(f"✅ Trainer {trainer_id} - {len(trainer_data)} course records found!")
        else:
            print(f"⚠️ No course stats found for trainer {trainer_id}. Skipping.")
    ...
    if results:
        ...
        print(f"✅ Saved {len(results)} trainer course stats to {OUTPUT_FILE}")
    else:
        print("⚠️ No trainer course stats found.")
